...
$(document).ready(function () {
    ...
    $(document).on('focusout', '.fac-name', function () {
        ...
        if (value == '') {
            errorBlock.show()
            if (!errorList3.includes(errorMsg)) {
                errorList3.push(errorMsg)
            }
        } else {
            if (errorList3.includes(errorMsg)) {
                let errIdx = errorList3.indexOf(errorMsg)
                errorList3.splice(errIdx, 1)
                errorBlock.hide()
            }
        }
        // lg(errorList)
    ...
    $(document).on('focusout', '.std-cap', function () {
        ...
        if (value == '') {
            //check if num error is in array, then remove if //true
            let errorTxt = "Number is require for student capacity " + numData
            if (errorList3.includes(errorTxt)) {
                let errIdx = errorList3.indexOf(errorTxt)
                errorList3.splice(errIdx, 1)
            }
            errorBlock.show()
            if (!errorList3.includes(errorMsg)) {
    ...
    $('#v-pills-additional-tab').click(function () {
        ...
        } else if (errorList3.length > 0) {
            for (let idx = 0; idx < errorList3.length; idx++) {
                error = errorList3[idx]
                errDiv.append('<p style="display:block" class="invalid-feedback">. ' + error + '</p>')
            }
